...
**The report computes this and persists nothing.**

**The teardown a run prints for its own live overlay is the runtime's own removal, not a verb from this file.** A live overlay is never a target here under any flag, deliberately — its worktree is listed, so nothing in this file will stop or remove it. The Output Contract still owes the developer the exact command, and it is the runtime's, naming the container this run recorded and carrying **`-v`**.

**An overlay launched under its own project also materialises the project's NAMED volumes, and no query in this file can find them either.** `--no-deps` skips *starting* a dependency, never *declaring* it: a unit that names a store in `depends_on` and is run under a project of its own gets that store's named volume created under that project — empty, because nothing ever started to write to it. Measured: one `--no-deps` run of a unit depending on a store left `<project>_<volume>` behind, carrying `com.docker.compose.project` and `com.docker.compose.volume` and **not one label this skill writes**. So the scoped queries above return nothing for it, `report` cannot count it, and a developer accumulates one per overlay project with nothing able to enumerate them.

**It is found by the project prefix, which is the only name it has.** The run knows that prefix because it chose it, so the Output Contract's teardown names these volumes explicitly alongside the container — the same obligation the anonymous volume carries below, for the same reason and by a different handle. What this file will not do is guess: an unlabelled volume nobody recorded a project for is another consumer's, and the rule against acting without proof of ownership governs here exactly as it governs a process. **A run that cannot state its own project prefix has left objects it must report as unremovable rather than sweep by pattern.**

**Name the anonymous volume in the report as well as carrying `-v`.** A unit that runs a store's image for a client binary it needs — `psql` to reach a database, and nothing about the image's own server — inherits that image's declared volume as an anonymous one, created by the launch and belonging to nobody. `-v` takes it, which is why the command carries it; naming it is what lets a developer who removed the container by hand, without the flag, find the object afterwards. Without the name it is unreachable: it carries none of this skill's labels, so no scoped query returns it, and an unscoped `docker volume ls` on a working machine returns thousands.

**Unknown has two readings and the report distinguishes them.** A store that could not be read is a failure: the held-port set is short of what is really held, and a later pick can be handed a port something already holds. A store that was never written is not — nothing has registered a host launch, so there is nothing to read and nothing missing, and the held set covers container overlays completely. The first says *could not be read*; the second says the set covers container overlays only. Collapsing them sends a developer after a file whose absence is the answer.

**A repository with no host-kind unit therefore never reaches zero-checked, and that is a property of the repository rather than a fault in the run**: nothing will ever register a host launch, so the sidecar is never written and every report says `registry-missing` for as long as the repository stays container-only. **It is the honest answer — unknown is not zero, and this is the shape where unknown is permanent — so read it as *this repository runs nothing on the host* rather than as a store that failed to load, and do not go looking for a file whose absence is the correct state.** The next legitimate write — a launch registration, or a completed mutation — commits the reconciled set through the write discipline of section 7. A pass that rewrote a file on every invocation would need the lock on every invocation, and "the report acquires no lock" would stop being true.
