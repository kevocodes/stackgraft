...
(Verify: file review of `references/shared-state.md` and `../isolation-providers/spec.md` — the in-instance rules are present and the substrate namespace table is gone; a writing pair exercised with and without a declared lifecycle target; the cases-with-no-safe-answer list diffed against `1.1.0`.)
