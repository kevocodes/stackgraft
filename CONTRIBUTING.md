...
The eleventh needs no runtime at all: it asks whether everything the documents tell an agent to reach exists and behaves as written, which is the necessary condition under "an agent can drive this" and the only half of it testable without a model in the loop. The other ten need a container runtime and skip by name without one; `STACKGRAFT_REQUIRE_RUNTIME=1` makes that skip fatal, which is what CI sets.
