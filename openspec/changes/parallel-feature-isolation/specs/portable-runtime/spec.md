...
**This change's net effect on the body MUST be negative.** The per-substrate namespace table MUST leave the body entirely and the provider contract MUST never enter it: a change that replaces the isolation half and adds a scope statement pays for the addition out of the replacement, not out of the ceiling. The body MUST state that a verdict is required and where the procedure lives, and MUST NOT state any condition under which an overlay is permitted (`../shared-state-safety/spec.md`).
