...
**A published copy holds a host port, and the run that made it is the only thing that knows.** The runtime picks it, so it appears in no manifest: it is per-worktree and per-provision, exactly like the instance name, and `portPolicy` holds the base stack's ports rather than this run's. But it is a real allocation on the developer's machine, and a later run that does not know about it can be handed it as a candidate — `scripts/pick-port.sh` excludes what it is given and nothing else. So **a run reports the ports its own copies publish and passes them as exclusions when it picks**, taking each from the `port` record `address` returns for that copy; a copy this run did not make is found the same way, by asking `address` for the copies the report already listed. Not tracked in the cache, because a value the runtime chooses at provision time is not a fact about the repository — recovered from the runtime instead, which is where every other engine-specific fact here comes from.

**The copy is published on loopback and nowhere else.** Publishing every exposed port to every interface is what a runtime does by default when asked to publish at all, and it is the wrong default here twice over: the copy holds the same bytes as the base store and the same credentials open it, while the store it was copied from commonly binds loopback and nothing else — so the duplicate would be reachable from addresses the original refuses. One flag per exposed port, each bound to `127.0.0.1` with the host port left empty, keeps the runtime as the only allocator, which is the property publishing-all was there for.
...
| an empty instance of the same image | `docker run -d --rm --label "stackgraft.repo=$hash8" --label "stackgraft.worktree=$worktree" --label "stackgraft.probe=$store" --env-file "$baseEnv" "$image"` | `docker exec "$instance" "$@"` |
...
**The route above is rung 1's, and the table would otherwise read as the only one there is.** A rung-1 candidate is a vector already inside the image, so `docker exec` is how it is reached. A rung-2 candidate is a **file in the repository**, which is not inside that image and never will be: issued through the column above it resolves to nothing, and the pair fails for a reason that has nothing to do with the data either. It is issued on the host instead, as the read file itself with the instance as its one argument — `<the repository's read file> "$instance"`, the executable invoked directly and **never through `sh`**, because the template contract rejects a shell as the program outright and governs this vector unchanged. A generated family is written executable for exactly this reason; a read file that is not executable supplies no vector and the rung is empty — `references/shared-state.md` states that where the generated family is defined. **What "one route" governs is the three issues of a single candidate, never the two rungs**: whichever route a candidate's rung selects, the base store, the empty instance and the copy are all read through that one, and a run that mixed them would manufacture the very difference it is reading for.

- **A read that failed produced no output, and a difference measured against one is not discrimination.** The three issues of a candidate are comparable only where all three *answered*. A command that exits non-zero, or whose route could not reach the instance, has produced nothing to compare — and treating its failure as one side of the difference is how a command that answers identically everywhere gets recorded as a query. The empty instance is the issue most exposed to it, because it is the one that was started a moment ago. Where any of the three did not answer, the candidate is **undetermined rather than discriminating**: retry it, and where it still does not answer, refuse the pair for want of a query. Certifying on a difference nothing produced is the one outcome this table exists to prevent, and it is indistinguishable from success on the run that does it.
...
**The empty instance carries no state and is removed by the runtime rather than by a query.** It mounts nothing, so it can hold nothing; it is started with `--rm`, so the runtime takes it and its anonymous volumes away when it stops and **no command here ever names an object to remove**.

**It is given the environment the base container runs under, and that is what the rule above already required rather than an exception to it.** Emptiness is a property of what an instance is *given* — no volume, no state — never of what it is *told*, and a store image routinely refuses to initialise at all without one: `postgres` exits `1` with *"Database is uninitialized and superuser password is not specified"* before any query could be issued against it. An instance that never booted is not an empty instance, it is an absent one, and the comparison it exists for cannot be made against nothing — so the copy is never certified, and the pair refuses however good the copy actually is. The environment is read back from the base container exactly as the image, the mount point and the command are, and passed as a file rather than expanded on a command line, because a value holding whitespace — and the shipped `postgres` image bakes one in — is one argument to the runtime and several to a shell. It carries no volume, so what it initialises is the empty shape of the store and never its contents: the discriminator's whole job is to tell that shape from the copy, and it does — `2` against `0` on the fixture this is exercised against. It carries this repository's hash and the worktree so that a run which died still leaves something a person can find, and it deliberately carries no `stackgraft.store`: the complete four-label set is what a copy *is*, and a probe that answered the copy's own query would be a copy nothing ever seeded.
