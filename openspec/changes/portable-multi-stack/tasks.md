...
- [x] 2.6 Rewrite `references/discovery.md`: split §(a) path→unit from §(b) unit→launch and tag every source with the question it answers; add the R1/R2/R3 resolver ladder led by `docker compose config --no-interpolate --format json`, the never-execute-repo-code rule, and degradation that never hard-fails; add the DS17 `covers` rule (dotted paths, subsumption normalisation, coarse-wins, evidence keys invalid, evidence invalidation); add `revalidate: always`. Add both `covers` granularities to the example. *Verify: V1-style file review; V3.*
- [x] 2.7 Reconcile `references/traps.md:29` with the R1 resolver: `--no-interpolate` does not print secrets, so it is exempt from the permission-blocked-config trap. Travels with 2.6 because it is only true once R1 exists. *Verify: file review against `discovery.md` §(b).*
