...
- **`dependsOn`** — every dependency the unit reaches. Each name must resolve to a `services` or a `backingStores` entry; a name in neither is unknown and fails closed. **Required on every runnable unit, and the gate fires on it, so an incomplete `dependsOn` is a silently ungated overlay.** An empty array means checked-and-none and must carry `stateReview` like any other classification claim. Collect it from every source, not just the orchestrator's own `depends_on`: a peer named only in `peerEnv`, a connection string in `.env.example`, or a client the code constructs is a dependency too. When you cannot prove the list is complete, say so — an unproven list is not a verified-empty one.
