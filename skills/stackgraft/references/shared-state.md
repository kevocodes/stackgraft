...
**That record is evidence, so it expires like every other one.** Every sibling has something whose drift re-derives it — a source has its `fingerprint`, an isolation `approval` has `sourceFingerprint`, a classification has `stateReview.serviceFingerprint` — and a record with no trigger is a verdict written once and never revisited, not even after the repository gains a store. So at least one `sources[]` entry MUST `cover` `backingStores`; the schema rejects the record without one. The record is a claim *about* that map, so the fingerprint of the file the map was read from is its expiry. **A root record whose covering source has drifted is stale, and a stale record is not a verdict** — the emptiness is unevidenced again until this run re-establishes it, which is undetermined, which is step 1, which refuses. `references/discovery.md` section 5 re-derives the record whenever it re-derives `backingStores`, and `stateReview` is a `covers` token in its own right for a source that establishes the absence directly.
