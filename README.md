...
- **A writing pair now isolates onto a copy of the store's state, and that copy is NOT YET VERIFIED. This step alone must not be released.** `scripts/provider-docker.sh` ships: it copies a store's state, starts an instance on the copy, says how to reach it, and removes both again — all of it label-scoped, all of it refusing rather than guessing. What is missing is the step that proves the copy carries the data it was seeded with. **Until that lands, a copy that started is being treated as a copy that works, and an unverified copy the overlay then writes into is precisely the false green this whole change exists to prevent.** Read every claim below about copies as describing a mechanism that is built and not yet trusted.
