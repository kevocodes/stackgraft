...
## 9a. Store copies: specified here, and NOT reclaimed by this script

**Read this before anything below it. `scripts/reap.sh` has no copy target.** It accepts `c:` containers and `p:` processes and nothing else — no volume query, no `stackgraft.store` label read, no copy line in the report. So every rule in this section is a **specification of what copy reclamation must do when it is built**, and not a description of what runs today. Nothing in this skill reclaims a copy whose worktree is gone.

What does ship, and is worth knowing instead:

- **`provider-docker.sh destroy <hash8> <worktree> <store>` removes the instance and the copy** for a worktree you can still name. It proves ownership by the complete label set and by worktree equality, and it refuses anything it cannot read. **It takes no flags**, so nothing in the provider stands between a `destroy` invocation and the copy — the interlock below is the agent's procedure, not the script's.
- **Every launch prints the copy's name and the exact command that removes it**, so an orphaned copy is always reclaimable by hand and never unfindable.
- **An orphaned copy is not detected.** Nothing compares a copy's recorded worktree against git's worktree list, so a copy whose worktree you deleted stays on your disk, unreported, until you remove it with the command the run printed. On gigabyte volumes that is the cost of this gap, and it is stated here rather than left to be discovered.

The rest of this section is the design that gap will be closed against. A seeded copy — `references/isolation-providers.md` — is a runtime object this skill created, so it outlives its worktree exactly the way an overlay does, and it costs disk while it does. It is to be reclaimed under the same rules, with **one deliberate difference**.
...
**Removal is to take the removal flag *in addition to* the mutation flag** — the one rule above that no shipped code enforces, since `destroy` takes no flags at all. That is the difference, and it runs the opposite way to intuition: a copy is the more expensive thing to have destroyed by accident, not the less. An overlay container that should not have been stopped is restarted from its image in seconds; a copy that should not have been removed is a state nothing on this host can reproduce — the base stack has moved on since it was taken. So the cheap verb is not available for it. The instance running on a copy is an ordinary container target and follows the ordinary rules; the **copy itself** is what the second flag protects.

**An unlabelled object, or one whose `stackgraft.labels` value this run does not recognise, is reported and never acted on** — the same fail-safe direction section 14 takes for everything else it cannot read. A future label-set version means this run does not know what those labels mean, and acting on an object you cannot read is the failure this whole file is written against.
