...
- **The base stack is excluded by construction, and by its ports where that is not enough.** Only an overlay launch writes the ownership label, so a base-stack container is never a candidate to begin with. The one shape that gets past that — a container hand-labelled with this repository's id — is caught by the ports the manifest records for the base stack, which the run passes in. Those ports are evidence, so **a mutation given none of them refuses rather than guessing, and no flag stands in for one**: a run that cannot name a single base port does not mutate a container, and there is no override. What that leaves uncovered is one narrow case, named in `references/reaping.md` and in the requirement itself: a hand-labelled container on a port the run did not pass. Nothing else on a running container tells a base-stack service apart from an overlay.
