...
## What a first run costs you

**Two questions, and both are asked rather than guessed.** They are here so the decision to install is made with them rather than after them.

**One port range, once per repository.** Nothing in your repository says which host ports an overlay may take, and this skill will not guess — a range nobody sized for your machine is how an overlay lands on a port you were using. So the first run stops and asks, records the answer, and never asks again. Everything else it discovered is written down *before* it asks, so answering costs one question rather than a rediscovery.

**Per writing store, a read command — which it offers to write.** Below is why that command is needed and why your existing healthcheck probably cannot serve as one. On a repository with none, the first run for each writing store provisions a copy, finds no query, **destroys the copy and refuses that pair**, and then offers three files. Approve them once and the refusal is over for good.

**Which adds up to:** one answer, plus three files per writing store you actually overlay. After that, runs are silent.
...
- **The verification is scripts driving the mechanism rather than an agent reading the skill.** Until recently this entry said the gate had never been exercised against a production-shaped repository at all, and the two defects that admission was hiding are in `2.1.1`: the empty instance every copy is compared against was launched with no environment and never booted, so no copy could be certified however faithful it was. Six layers now run every build — **849** prose and schema checks, then **219** behavioural ones that boot real stores under their own entrypoints and drive both hazards the gate names, both roads that answer the first of them, the manifest discovery produces, the generated family with both of its falsifiers, two worktrees at once, the actuator that stops and removes, and the refusal a repository with no read command actually reaches. Blindness to the substrate is **demonstrated rather than argued**, and by the only evidence that settles it: one of those runs drives an engine **whose name appears in no document here** — the run checks that first and aborts if any reference, asset or script has learned the word — and the whole road holds, because what changes between engines is the read command **the repository supplies** and nothing in the procedure changes at all. Four engines someone chose would pass a procedure tuned to those four; that is what the fifth is for, and it is what found the last defect on this list. The four are still there for what they disagree about: two refuse to boot without environment and two need none, one declares a `CMD-SHELL` healthcheck the argv rule excludes before anything else is asked and two declare exec-form vectors that *are* rung-1 candidates and are refused by the discriminator instead, two are relational, one is a keyspace and one a document store. The scripts also run under the three shells a contributor actually has — `dash` on a minimal image, bash 3.2 as macOS's `/bin/sh`, and Git Bash on Windows — because a shape all three of those disagree about is a script that parses in CI and not on a laptop. What remains is stated rather than implied: **the runs are driven by test scripts, so what is covered is that the mechanism holds and that the instructions are followable — never that an agent follows them well.** The followable half is checked and is not nothing: every path a shipped document points at resolves, every script it ships is named by a document rather than reachable only by listing the directory, every documented invocation runs and every malformed one fails loudly instead of silently, the derivation `SKILL.md` opens with reproduces from either checkout, and the placeholder set the example hands an agent is closed. Those are the failures that stop an agent before it reasons about anything. **The other half is not checked and has no instrument here**: whether an agent reading these documents picks the right step, reads the gate correctly, and refuses where it should needs a model in the loop, whose verdict is not an assertion. Nothing in this repository claims it.
