...
(Verify: file review plus POSIX syntax check under `dash -n` for all four scripts; portability grep for disallowed binaries; both new scripts executed on macOS and on a minimal Linux image; every path `with-lock.sh` creates compared against the four the carve-out names, the transient reclaim name included, and each confirmed gone once the run ends.)
