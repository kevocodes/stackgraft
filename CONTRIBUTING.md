...
.github/scripts/integration-theft.sh      # theft, the other hazard
.github/scripts/integration-reaping.sh    # the mutation: stopping and removing
.github/scripts/integration-noquery.sh    # what a real first run gets
.github/scripts/integration-unadvised.sh  # an engine no document in the skill names
.github/scripts/integration-staleness.sh  # does the overlay run the worktree's code
.github/scripts/integration-drivability.sh # the documents, against what they point at
```

**The first one and the other twelve answer different questions, and the difference is the reason the other twelve exist.** `verify.sh` checks the schema is valid, the example validates against it, every negative fixture is rejected, both scripts pass `dash -n` and actually run, the body is within budget and contains no permitting term, every manifest field named in a document exists in the schema, the four release version strings agree, the released version has a `CHANGELOG.md` entry that extracts to a usable release body, and no agent-specific coupling crept in. Almost all of that asks whether a document says a thing or whether a record validates — worth asking, and unable by construction to notice a recipe that does not run, because the rows check that the prose says what the prose says. A shipped recipe that exited `1` on the first line of a real store was pinned, carried a negative control, and had every one of those checks defending it.

Eleven of the twelve integration scripts boot real stores **under their own entrypoints** and drive the mechanism. An `--entrypoint` override is what hides an image's boot requirements, so they do not use one. Several engines rather than one because blindness to the substrate is a claim about the procedure, and one engine cannot evidence it however thoroughly that one is exercised. Nine of them drive `postgres`, `redis`, `mysql` and `mongo`, which differ in what the procedure must not care about: two refuse to boot without environment and two need none; one declares a `CMD-SHELL` healthcheck the argv rule excludes and two declare exec-form vectors that reach the discriminator and are refused there; two are relational, one is a keyspace and one is a document store. What changes between them is the read command the repository supplies.

Those four are still four engines someone chose, and a procedure tuned to them would pass all nine. So the tenth drives an engine **no document in this repository names** — every reference, asset and script is checked for the word first, and the run aborts if any of them has learned it. Its state is a store the structural pass finds and a name-matching pass misses, and the whole copy road runs against it: bytes copied, the repository's own read discriminating a base holding data from an empty instance, the copy answering what the base answers, the copy destroyed. That row is how the multi-element entrypoint came out — an image wrapping its start-up in an init is ordinary rather than exotic, `--entrypoint` takes one string, and the provider had been refusing the shape instead of rebuilding the argv. Four engines never asked.

The twelfth needs no runtime at all: it asks whether everything the documents tell an agent to reach exists and behaves as written, which is the necessary condition under "an agent can drive this" and the only half of it testable without a model in the loop. The other eleven need a container runtime and skip by name without one; `STACKGRAFT_REQUIRE_RUNTIME=1` makes that skip fatal, which is what CI sets.
